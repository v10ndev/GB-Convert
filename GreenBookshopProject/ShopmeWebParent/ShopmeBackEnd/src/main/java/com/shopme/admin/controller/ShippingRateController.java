...
public class ShippingRateController {
	...
	private String defaultRedirectURL = "redirect:/shipping_rates/page/1?sortField=province&sortDir=asc";
	...
	public String newRate(Model model) {
		...
		List<Province> listProvinces = service.listAllProvinces();
		
		LOGGER.info("ShippingRateController | newRate | listProvinces : " + listProvinces.toString());
	...
	public String editRate(@PathVariable(name = "id") Integer id,
		...
		try {
			...
			LOGGER.info("ShippingRateController | editRate | listProvinces : " + listProvinces.toString());
	...
	public String updateCODSupport(@PathVariable(name = "id") Integer id,
		...
		try {
			...
			ra.addFlashAttribute("message", "COD hỗ trợ giá cước vận chuyển ID " + id + " đã được cập nhật.");
		} catch (ShippingRateNotFoundException ex) {
	...
	public String deleteRate(@PathVariable(name = "id") Integer id,
		...
		try {
			...
			ra.addFlashAttribute("message", "Giá vận chuyển ID " + id + " đã bị xóa.");
		} catch (ShippingRateNotFoundException ex) {
