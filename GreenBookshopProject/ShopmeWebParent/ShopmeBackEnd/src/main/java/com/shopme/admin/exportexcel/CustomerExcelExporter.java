...
public class CustomerExcelExporter extends AbstractExporter{
	...
	private void writeHeaderLine() {
		...
		createCell(row, 4, "Commune", cellStyle);
		createCell(row, 5, "District", cellStyle);
		createCell(row, 6, "Province", cellStyle);
	...
	private void writeDataLines(List<Customer> listCustomers) {
		...
		for (Customer customer : listCustomers) {
			...
			createCell(row, columnIndex++, customer.getCommune(), cellStyle);
			createCell(row, columnIndex++, customer.getDistrict(), cellStyle);
			createCell(row, columnIndex++, customer.getProvince().getName(), cellStyle);
