...
public class ReviewController {
	...
	public String saveReview(Review reviewInForm, RedirectAttributes ra) {
		...
		ra.addFlashAttribute("messageSuccess", "Đánh giá ID " + reviewInForm.getId() + " đã được cập nhật thành công.");
		return defaultRedirectURL;		
	}
	...
	public String deleteReview(@PathVariable("id") Integer id, RedirectAttributes ra) {
		...
		try {
			...
			ra.addFlashAttribute("messageSuccess", "Đánh giá ID " + id + " đã bị xóa.");
		} catch (ReviewNotFoundException ex) {
